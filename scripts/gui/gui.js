...
world.beforeEvents.playerInteractWithEntity.subscribe((ev) => {
  try {
    ...
    if (config.entities) {
      for (const gName of Object.keys(config.entities)) {
        const group = config.entities[gName];
        if (group && Object.prototype.hasOwnProperty.call(group, entity.typeId ?? entity.id ?? entity.__identifier__)) {
          entGroup = gName;
          break;
        }
      }
    ...
    const typeId = entity.typeId ?? entity.id ?? entity.__identifier__ ?? null;
