...
export function registerSoldierSystem(cfg) {
    ...
    system.run(() => {
        ...
        if (loaded)
            systemStates[cfg.command] = loaded;
    });
    ...
    system.beforeEvents.startup.subscribe((init) => {
        ...
        init.customCommandRegistry.registerCommand(toggleCommand, (origin, faction, enable, includeSpecial) => {
            ...
            enable = enable !== null && enable !== void 0 ? enable : true;
            includeSpecial = includeSpecial !== null && includeSpecial !== void 0 ? includeSpecial : false;
    ...
    function handleSoldierEntity(ent) {
        ...
        if (!isSpecialFoundation && !isSpecialChaos && team !== "foundation" && team !== "chaos")
            return;
        if (!allSoldiers.includes(ent.id))
            allSoldiers.push(ent.id);
...
//# sourceMappingURL=toggle_system.js.map