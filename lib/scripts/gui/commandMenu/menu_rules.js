...
export function canApplySystem(systemId, allSystemStates, faction, isSpecial) {
    ...
    for (const [requiredDomain, allowedValues] of Object.entries(rules.requires)) {
        ...
        if (!isAllowed) {
            debugWarn("menuRules:compat", `Sistema ${systemId} NO puede aplicarse: ${requiredSystemId} debe ser ${JSON.stringify(allowedValues)}, pero es ${currentValue}`, "yellow");
            return {
                canApply: false,
...
export function shouldApplyToFutureEntities(systemId) {
    ...
    debugWarn("menuRules:apply", `Sistema ${systemId}: applyMode=${rules.applyMode}, aplicar a futuras=${shouldApply}`, shouldApply ? "green" : "yellow");
    return shouldApply;
}
/**
...
//# sourceMappingURL=menu_rules.js.map