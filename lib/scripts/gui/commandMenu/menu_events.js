...
function handleSoldierEntity(ent) {
    var _a;
    try {
        if (!ent || !isValidSoldier(ent, specialUnits))
            return;
        ...
        debugWarn("menuEvents:spawn", `Nueva entidad: ${nameTag} [${faction}${isSpecial ? '-especial' : '-normal'}]`, "cyan");
        ...
        for (const systemId in systems) {
            ...
            if (!shouldApplyToFutureEntities(systemId)) {
                debugWarn("menuEvents:spawn", `Sistema ${systemId} NO se aplica a entidad futura (applyMode: existing_only)`, "gray");
                continue;
            }
...
//# sourceMappingURL=menu_events.js.map